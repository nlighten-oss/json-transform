import { areSimilar, cleanParsedSchemaProperty, inferJSONSchemaType, TypeSchema } from "@nlighten/json-schema-utils";
...
const isCustomJsonPath = (data: any) =>
  typeof data === "string" && (data.startsWith("$.") || data.startsWith("#") || data === "$");

function rawWalkOnObject(targetPath: string, data: any, s: string, paths: string[], context: ParseContext) {
  ...
  if (Array.isArray(data)) {
    for (let i = 0; i < data.length; i++) {
      rawWalkOnObject(targetPath, data[i], s + "[" + i + "]", paths, context);
    }
  ...
  if (data && typeof data === "object") {
    ...
    for (const p in data) {
      if (VALID_ID_REGEXP.test(p)) {
        rawWalkOnObject(targetPath, data[p], s + "." + p, paths, context);
      } else {
        rawWalkOnObject(
          targetPath,
          data[p],
...
const findNonNull = (value: string, context: ParseContext) => {
 ...
 */
function copyArrayItemTypeOnWalk(
  value: any[] | string,
  targetPath: string,
  localPath: string,
  previousPaths: string[],
  ...
  const dataWithResultType = Array.isArray(value)
    ...
    : previousPaths.includes(value + "[0]")
      ? value + `[${findNonNull(value, context) ?? 0}]`
      : value + "[]";
  parseTransformerInternal(dataWithResultType, targetPath, localPath, previousPaths, paths, context);
}

/**
 * Handle resolving of function output type
 */
const handleFunction: HandleFunctionMethod = (
  detectedAs,
  funcName,
  func,
  value,
  ...
  switch (funcName) {
    ...
    case EmbeddedTransformerFunction.flat: {
      copyArrayItemTypeOnWalk(value, targetPath, localPath, previousPaths, paths, context);
    ...
    case EmbeddedTransformerFunction.if: {
      if (typeof args.then !== "undefined") {
        parseTransformerInternal(args.then, targetPath, localPath, previousPaths, paths, context);
        break;
      }
    ...
    case EmbeddedTransformerFunction.reduce: {
      if (typeof args.identity !== "undefined") {
        parseTransformerInternal(args.identity, targetPath, localPath, previousPaths, paths, context);
      }
      break;
    }
    case EmbeddedTransformerFunction.value: {
      // input and output should have the same schema
      parseTransformerInternal(value, targetPath, localPath, previousPaths, paths, context);
      break;
    }
    case EmbeddedTransformerFunction.raw: {
      // input type is the result type
      rawWalkOnObject(targetPath, value, localPath, paths, context);
      break;
    }
    ...
    case EmbeddedTransformerFunction.lookup: {
      ...
      parseTransformerInternal(dataWithCurrentType, "##current", "", previousPaths, currentPaths, context);
          ...
          args.using.forEach((use: any) => {
            ...
            const dataWithMatchType = Array.isArray(use.with)
              ...
              : previousPaths.includes(use.with + "[0]")
                ...
                : use.with + "[]";
            parseTransformerInternal(dataWithMatchType, "##" + use.as, "", previousPaths, currentPaths, context);
          });

          const prevPaths = currentPaths.concat(previousPaths.filter(x => !x.startsWith("##current")));
          parseTransformerInternal(args.to, targetPath, localPath + "[]", prevPaths, paths, context);
        } else {
          args.using.forEach((use: any) => {
            if (typeof use.as !== "string") return; // invalid
            const dataWithMatchType = Array.isArray(use.with)
              ...
              : previousPaths.includes(use.with + "[0]")
                ? use.with + "[0]" // this is not the best approach (other indices may have different fields)
                : use.with + "[]";
            parseTransformerInternal(dataWithMatchType, targetPath, localPath + "[]", previousPaths, paths, context);
          });
    ...
    case EmbeddedTransformerFunction.transform: {
      ...
      if (funcName === EmbeddedTransformerFunction.map) {
        ...
        // at index 0
        parseTransformerInternal(itemsData, "##current", "", previousPaths, currentPaths, context);
      }

      const prevPaths = currentPaths.concat(previousPaths.filter(x => !x.startsWith("##current")));
      parseTransformerInternal(dataWithResultType, targetPath, localPath + targetSuffix, prevPaths, paths, context);
    ...
    case EmbeddedTransformerFunction.switch: {
      if (typeof args.default !== "undefined") {
        parseTransformerInternal(args.default, targetPath, localPath, previousPaths, paths, context);
        break;
      } else {
 ...
 */
const parseTransformerInternal: ParseMethod = (definition, targetPath, localPath, previousPaths, paths, context) => {
  ...
  if (isCustomJsonPath(definition)) {
    ...
    if (jsonPathFunctionSchema) {
      ...
      copySubPathsOnWalk(definition, targetPath, localPath, previousPaths, paths, context);
  ...
  if (
    functions.matchInline(definition, (funcName, func, value, args) => {
      // TODO: in the future to combine inline and object functions the 'definition' argument can be made from the args
      // //create new context for handling internal function
      // const result2 = structuredClone(context);
      // if (result2) {
      //   Object.keys(result2.paths).forEach(key => {
      //     if (key.startsWith("##current")) {
      //       delete result2.paths[key];
      //     }
      //   });
      // }
      // const previousPaths2 = previousPaths.filter(x => !x.startsWith("##current"));
      // const paths2 = paths.filter(x => !x.startsWith("##current"));
      // parsed inside the match function and each index will be mapped to the name of that argument
      handleFunction("inline", funcName, func, value, args, targetPath, localPath, previousPaths, paths, context);
    })
  ) {
  ...
  if (definition && typeof definition === "object" && !Array.isArray(definition)) {
      ...
      for (const p in definition) {
        if (p === "*") {
          if (isCustomJsonPath(definition["*"])) {
            ...
            definition["*"].forEach(
              pc =>
                isCustomJsonPath(pc) && copySubPathsOnWalk(pc, targetPath, localPath, previousPaths, paths, context),
            );
          }
          continue;
        }
        const propPath = VALID_ID_REGEXP.test(p) ? "." + p : "[" + (ALL_DIGITS.test(p) ? p : JSON.stringify(p)) + "]";
        parseTransformerInternal(definition[p], targetPath, localPath + propPath, previousPaths, paths, context);
        if (context.paths) {
    ...
    for (let i = 0; i < definition.length; i++) {
      parseTransformerInternal(definition[i], targetPath, localPath + "[" + i + "]", previousPaths, paths, context);
    }
    if (context.paths) {
...
export function parseTransformer(
  ...
  return parseTransformerInternal(definition, targetPath, "", previousPaths, paths, context);
}
