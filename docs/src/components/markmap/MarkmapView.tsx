...
Promise.all([loadCSS(styles),
  loadJS(scripts, { getMarkmap: () => markmap })
]).catch(console.error);

export default function MarkmapView({ root, options, ...rest}: {root: IPureNode, options?: Partial<IMarkmapOptions>} & JSX.IntrinsicElements["svg"]) {
