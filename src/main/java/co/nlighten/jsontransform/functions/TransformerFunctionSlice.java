...
public class TransformerFunctionSlice<JE, JA extends Iterable<JE>, JO extends JE> extends TransformerFunction<JE, JA, JO> {
    ...
    public Object apply(FunctionContext<JE, JA, JO> context) {
        ...
        if (begin >= 0) {
            ...
            value.stream(begin.longValue(), null).forEach(item -> ARRAY.add(result, item));
            for (int i = 0; i < -end; i++) {
                ARRAY.remove(result, ARRAY.size(result) - 1);
            }
            return result;
        }
        // begin < 0
        ...
        var arrSize = ARRAY.size(arr);
        var result = ARRAY.create();
        for (int i = arrSize + begin; i < arrSize + (end == null ? 0 : end) ; i++) {
            ARRAY.add(result, ARRAY.get(arr, i));
        }
        return result;
    }
}
